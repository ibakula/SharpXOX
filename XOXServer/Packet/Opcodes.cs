...
namespace XOXServer
{
    ...
    public struct OpcodesHandler
    {
        ...
        public static void Init()
        {
            ...
            handlerTable[1].handler += delegate(Connection client, Packet pckt) { client.HandleNullOpcode(pckt); };
            handlerTable[2].Name = "LOBBY";
            handlerTable[2].handler += delegate(Connection client, Packet pckt) { client.HandleNullOpcode(pckt); };
            handlerTable[3].Name = "START";
            handlerTable[3].handler += delegate(Connection client, Packet pckt) { client.HandleNullOpcode(pckt); };
            handlerTable[4].Name = "TURN";
            handlerTable[4].handler += delegate(Connection client, Packet pckt) { client.HandleNullOpcode(pckt); };
        }
        public string Name;
